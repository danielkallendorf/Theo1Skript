...
\begin{itemize}
\item $\vec F(t)$ Kraftkurve
\item $\vec x_1(t)$ Bahnkurve (ohne Zeitinformation: Bahnkurve)
\item Spezielle Kraftfelder:\\
$\vec K(\vec x,\dot{\vec x},t)\rightarrow \vec F(t)=\vec K(\vec x(t),\dot{\vec x}(t),t)$\\
\subitem\underline{Newton}: $\vec K(\vec x(t),\dot{\vec x}(t),t)=-\gamma \dfrac{\vec x_1-\vec x_2}{\norm{\vec x_1-x_2}^3}m_1^{(T)}m_2^{(s)}$\\
\subitem $m^{(s)}$: schwere Masse
\subitem $m^{(t)}$: träge Masse
\subitem \fbox{$m^{(T)}\equiv m^{(s)}$}
\end{itemize}
\begin{align*}
...
\subitem $\vec L_{rel}\cdot \vec r(t)=0\rightarrow $ ebene Dynamik
\subitem $\dfrac{d}{dt}\mu\vec r\times\dot\vec r=0$ Flächensatz
...
\draw[thick,->] (0)--(0,2) node[above] {$U_{eff}$};
\draw[thick] (0)--(0,-2.3);
\draw[dashed] (E0)--(4,-2);
\draw[dashed] (E1)--(4,-1);
\draw[dashed] (E2)--(4,);
...
\end{tabular}
%VL 20.10.2015
\subparagraph*{Funktional:} Jeder möglichen Bahnkurve wird eine Zahl zugeordnet.\\
...
&\equiv -\delta\int_{t_1}^{t2}dt\quad\vec F_i(t)\cdot \vec x_i(t)
\end{align*}
\end{proof}
...
\end{itemize}\subsection{Legendre Transformation}
...
\begin{proof}
betrachte virtuelle Verrückung $\big(\delta p(t_1),\delta q(t_1)\big)$\begin{align*}
